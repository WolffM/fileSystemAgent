"""Manages external audit tool binaries — discovery, verification, download."""
...
DEFAULT_TOOLS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "direct_url": "https://download.sysinternals.com/files/Sysmon.zip",
        "requires_admin": True,
    ...
    },
    {
        ...
        "install_method": "direct_url",
        "direct_url": "https://download.sysinternals.com/files/Sigcheck.zip",
    ...
    },
    {
        ...
        "install_method": "direct_url",
        "direct_url": "https://download.sysinternals.com/files/ListDlls.zip",
        "requires_admin": True,
        "license": "Proprietary (Microsoft)",
    },
]
...
class ToolManager:
    """Manages external audit tool binaries.

    Resolution order for finding a tool:
    1. Explicit path from config (audit.tools.<name>.path)
    ...
    def get_tool_path(self, tool_name: str) -> Path:
        ...
        if not tool.installed or tool.path is None:
            raise FileNotFoundError(
                ...
                f"Run 'python main.py audit setup' to install, "
                f"or set audit.tools.{tool_name}.path in config.yaml."
            )
        return tool.path
    ...
    async def _download_direct_url(self, tool_name: str) -> bool:
        ...
        url = tool.direct_url
        if not url:
            logger.warning(f"{tool.display_name}: no direct_url configured")
            return False
        ...
        try:
            async with httpx.AsyncClient(follow_redirects=True, timeout=120) as client:
                logger.info(f"Downloading {tool.display_name} from {url}")
                resp = await client.get(url)
                resp.raise_for_status()

                filename = url.rsplit("/", 1)[-1]
                asset_path = dest_dir / filename
