"""End-to-end tests that run real audit tool binaries against sandbox artifacts.

These tests require tools to be downloaded first via `python main.py audit setup`.
Tools that are not installed are automatically skipped.

Run with: pytest tests/audit/test_e2e_sandbox.py -v -m e2e
"""
...
from src.audit.scanners.chainsaw import ChainsawScanner
from src.audit.scanners.hayabusa import HayabusaScanner
from src.audit.scanners.sysinternals import SigcheckScanner
...
class TestToolAvailability:
    """Verify that audit setup downloaded expected tools."""
    ...
    def test_audit_check_cli(self):
        """Verify the CLI check command runs without error."""
        import subprocess
        result = subprocess.run(
            ["python", "main.py", "audit", "check"],
        ...
        assert "Audit Tool Status" in result.stdout
...
class TestCLIE2E:
    ...
    def test_cli_scan_dry_run(self):
        import subprocess
        result = subprocess.run(
            ["python", "main.py", "audit", "scan", "--dry-run"],
            cwd=str(PROJECT_ROOT),
            capture_output=True,
            text=True,
            timeout=30,
