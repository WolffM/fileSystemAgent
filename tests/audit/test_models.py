...
class TestFindingDomain:
    def test_default_is_security(self):
        f = Finding(
            tool_name="test", severity=SeverityLevel.LOW,
            category="test", title="t", description="d", target="x",
        )
        assert f.domain == FindingDomain.SECURITY
    ...
    def test_explicit_hygiene(self):
        f = Finding(
            tool_name="test", severity=SeverityLevel.LOW,
            category="test", title="t", description="d", target="x",
            domain=FindingDomain.HYGIENE,
        )
        assert f.domain == FindingDomain.HYGIENE

    def test_backward_compat_json(self):
        """Existing JSON without domain field should deserialize to SECURITY."""
        data = {
            "tool_name": "test", "severity": "low",
            "category": "test", "title": "t",
    ...
    def test_all_domain_values(self):
        expected = {"security", "performance", "hygiene"}
        actual = {d.value for d in FindingDomain}
        assert actual == expected
